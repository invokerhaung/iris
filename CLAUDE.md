...
| Store | 持久化 | 用途 |
|---|---|---|
| `AppStore` | ✅ | 主题、语言、音量、倍速、播放器后端等全局设置 |
...
GitHub Actions（`.github/workflows/ci.yml`）：push 到 `main`/`dev` 或 PR 时触发，构建 Windows 和 Android 产物；push 到 `main` 时自动创建 GitHub Release。
